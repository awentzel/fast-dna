...
) => (designSystem: DesignSystem) => Swatch = memoize(
    (targetContrast: number): ((designSystem: DesignSystem) => Swatch) => {
        return memoize(
            (designSystem: DesignSystem): Swatch => {
                ...
                const direction: 1 | -1 =
                    neutralForegroundIndex <= Math.floor(neutralPaletteLength / 2)
                        ? 1
                        : -1;
                ...
                while (
                    inRange(
                        neutralForegroundHintIndex + direction,
                        0,
                        neutralPaletteLength
                    ) &&
                    contrast(
                        background,
                        neutralPalette[neutralForegroundHintIndex + direction]
                    ) > contrastTarget
                ) {
    ...
    function neutralForegroundHintInternal(arg: any): any {
        if (typeof arg === "function") {
            return ensureDesignSystemDefaults(
                (designSystem: DesignSystem): Swatch => {
                    return neutralForegroundHintAlgorithm(contrastTarget)(
